...
// To easily parallelize requests
...
(async () => {
    ...
    // You can increase this parameter, but be respectful of the website you're crawling
    const concurrency = 3;

    const numPagesToCrawl = 10;
    ...
    const results = [];
    const pageIndexes = Array.from({ length: numPagesToCrawl }, (_, i) => i + 1)

    // Make 'concurrency' requests in parallel
    await Promise.map(pageIndexes, async(indexPage) => {
        try {
    ...
    */
    ...
    fs.writeFileSync('./result_crawl_parallel_hn.csv',
        results.join('\n', 'utf8'));
